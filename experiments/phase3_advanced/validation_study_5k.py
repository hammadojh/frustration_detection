#!/usr/bin/env python3
"""
Validation Study with 5000 Balanced Datapoints
Scale up the comprehensive ablation study to validate findings with even larger dataset
"""
...
def create_balanced_dataset_5k():
    """Create balanced 5000-sample dataset from EmoWOZ"""
    logger.info("Loading EmoWOZ dataset for 5k validation...")
    ...
    # Balance the 5k subset
    target_frustrated = min(2500, len(frustrated_examples))
    target_not_frustrated = min(2500, len(not_frustrated_examples))
    ...
    logger.info(f"5k Dataset created: {target_frustrated} frustrated, {target_not_frustrated} not frustrated")
...
def save_5k_dataset(examples, output_dir="data"):
    """Save 5k dataset for reproducibility"""
    os.makedirs(output_dir, exist_ok=True)
    
    # Save processed examples
    with open(os.path.join(output_dir, 'subset_5k_processed.json'), 'w') as f:
    ...
    with open(os.path.join(output_dir, 'subset_5k_stats.json'), 'w') as f:
        json.dump(stats, f, indent=2)
    
    logger.info(f"5k Dataset saved to {output_dir}")
    logger.info(f"Statistics: {stats}")

def load_data():
    """Load processed 5k examples"""
    processed_path = "data/subset_5k_processed.json"
    
    # Create dataset if it doesn't exist
    if not os.path.exists(processed_path):
        logger.info("5k dataset not found, creating it...")
        examples = create_balanced_dataset_5k()
        save_5k_dataset(examples)
    else:
        logger.info("Loading existing 5k dataset...")
    ...
    n = len(texts)
    train_end = int(0.7 * n)  # 70% train = 3500 samples
    val_end = int(0.85 * n)   # 15% val = 750 samples, 15% test = 750 samples
...
def extract_features_by_indices(texts, feature_indices):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        if (i // batch_size) % 10 == 0:  # Log every 10 batches for 5k
...
def test_early_fusion_combination(data, feature_indices, combo_name):
    ...
    def get_embeddings(texts):
        ...
        with torch.no_grad():
            for i in range(0, len(texts), batch_size):
                if (i // batch_size) % 20 == 0:  # Log every 20 batches for 5k
...
def main():
    logger.info("=" * 80)
    logger.info("VALIDATION STUDY: 5K BALANCED DATAPOINTS")
    logger.info("=" * 80)
    logger.info("This validation study will test key configurations from previous studies:")
    ...
    logger.info("  Total: ~19 focused experiments on 5k data")
    logger.info("")
    ...
    baseline_result = test_early_fusion_combination(data, [], "text_only_baseline_5k")
    results.append(baseline_result)
    logger.info(f"   5k Text-only F1: {baseline_result['test_f1']:.4f}")
    
    # 2. TOP INDIVIDUAL FEATURES (from previous studies)
    logger.info("\n2. Testing TOP INDIVIDUAL FEATURES from previous studies")
    # Based on consistent top performers: sentiment_slope, avg_politeness, avg_confusion, avg_negation
    top_individual_indices = [0, 2, 4, 5, 8]  # Top 5 from previous studies
    ...
    for i, idx in enumerate(top_individual_indices):
        ...
        result = test_early_fusion_combination(data, [idx], f"individual_{name}_5k")
        results.append(result)
    
    # 3. BEST BUNDLES (from previous studies)
    logger.info("\n3. Testing BEST BUNDLES from previous studies")
    bundle_indices = {
        'linguistic_bundle': list(range(0, 8)),      # Consistently best bundle
    ...
    for bundle_name, indices in bundle_indices.items():
        ...
        result = test_early_fusion_combination(data, indices, f"bundle_{bundle_name}_5k")
        results.append(result)
    
    # 4. KEY COMBINATIONS (from previous studies)
    logger.info("\n4. Testing KEY COMBINATIONS from previous studies")
    key_combinations = {
        'top_3_features': [0, 2, 4],                    # Top 3 individual (best performer on 2k)
        'top_8_features': [0, 2, 4, 5, 8, 9, 10, 11],  # Best cumulative from original ablation
        'linguistic_8': list(range(0, 8)),              # Linguistic bundle (consistently good)
    ...
    for combo_name, indices in key_combinations.items():
        ...
        result = test_early_fusion_combination(data, indices, f"{combo_name}_5k")
        results.append(result)
        
        # Save progress after each major test
        temp_df = pd.DataFrame(results)
        temp_path = f"results/validation_5k_progress_{len(results)}_tests.csv"
        os.makedirs("results", exist_ok=True)
        temp_df.to_csv(temp_path, index=False)
        logger.info(f"     Progress saved: {len(results)} tests completed")
    
    # 5. LEAVE-ONE-OUT for critical features (from previous studies)
    logger.info("\n5. Testing LEAVE-ONE-OUT for most critical features")
    # Test removing the most critical features found in previous studies
    ...
    for i, feat_idx in enumerate(critical_features_to_remove):
        ...
        result = test_early_fusion_combination(data, remaining_features, f"loo_remove_{name}_5k")
        results.append(result)
    ...
    csv_path = f"{output_dir}/validation_study_5k.csv"
    df.to_csv(csv_path, index=False)
    
    # Create comparison report
    report_path = f"{output_dir}/validation_study_5k_REPORT.txt"
    
    with open(report_path, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("VALIDATION STUDY: 5K BALANCED DATAPOINTS\n")
        f.write("=" * 80 + "\n")
        f.write(f"Total validation experiments: {len(results)}\n")
        f.write(f"Dataset size: 5000 balanced examples (2500 frustrated, 2500 not frustrated)\n")
        f.write(f"Train/Val/Test split: 3500/750/750\n\n")
        
        # Baseline comparison
        baseline_f1 = baseline_result['test_f1']
        f.write(f"5K BASELINE PERFORMANCE:\n")
        f.write(f"  Text-only F1: {baseline_f1:.4f}\n\n")
        
        # Sort results by F1 score
        results_sorted = sorted(results, key=lambda x: x['test_f1'], reverse=True)
        
        f.write("TOP PERFORMING CONFIGURATIONS (5K validation):\n")
        f.write("-" * 60 + "\n")
        ...
        f.write("• 2k-sample text-only baseline: 0.7606\n")
        f.write(f"• 5k-sample text-only baseline: {baseline_f1:.4f}\n")
        f.write(f"• 2k to 5k baseline difference: {baseline_f1 - 0.7606:+.4f}\n\n")
        
        # Best configuration
        best_result = results_sorted[0]
        f.write(f"• Best 5k configuration: {best_result['combination']}\n")
        f.write(f"• Best 5k F1 score: {best_result['test_f1']:.4f}\n")
        f.write(f"• Improvement over 5k baseline: {best_result['test_f1'] - baseline_f1:+.4f}\n")
    ...
    logger.info(f"  5k Text-only baseline: {baseline_result['test_f1']:.4f}")
    logger.info(f"  Best 5k configuration: {best_result['combination']} (F1: {best_result['test_f1']:.4f})")
